...
final class RecallController {
    ...
    func status() -> [String: Any] { ["ok": true, "configured": configured, "region": region, "jobs": jobs] }
    func handle(_ action: String, _ body: [String: Any], model: AppModel) async throws -> [String: Any] {
        ...
        case "add":
            var meeting = try required(body, "meetingUrl")
            if URL(string: meeting)?.scheme != "https" {
                let id = Self.meetingID(meeting)
                _ = try await handle("list", [:], model: model)
                guard let resolved = knownMeetingURLs[region + ":" + id] else {
                    throw AppError("No saved join details were found for this meeting ID. Use Change meeting to paste its invite link once; the link includes any required passcode.")
                }
                meeting = resolved
            }
        ...
        case "speak":
            ...
            jobs.append(["id": id, "botId": bot, "text": text, "at": ISO8601DateFormatter().string(from: at), "status": "preparing", "sequence": jobs.count, "dispatched": 0, "loop": loop])
            dueTimes[id] = at
            let voice = body["voice"] as? String ?? model.voiceID
            let speechModel = model.modelID
            let recallKey = key!
            let recallRegion = region
            tasks[id] = Task { await self.run(id: id, bot: bot, text: text, voice: voice, model: speechModel, at: at, count: loop ? nil : count, gap: gap, recallKey: recallKey, recallRegion: recallRegion) }
    ...
    private func run(id: String, bot: String, text: String, voice: String, model: String, at: Date, count: Int?, gap: Double, recallKey: String, recallRegion: String) async {
        var acquired = false
        ...
        do {
            ...
            while count == nil || pass < count! {
                ...
                try await sleep(duration + 2 + gap)
            }
