...
- **macOS command line tool** (optional, for scripts and LLM agents): install or update it with one line. It downloads the signed `botspeaker` binary attached to the latest release.
  ...
  ```
- **Windows 10/11 x64:** [BotSpeaker 0.5.2 installer](https://github.com/DJBen/BotSpeaker/releases/download/0.5.2/BotSpeaker.Windows-win-Setup.exe), with automatic update downloads and Desktop/Start menu shortcuts. Existing portable users should install once. A [portable ZIP](https://github.com/DJBen/BotSpeaker/releases/download/0.5.2/BotSpeaker-Windows-x64-0.5.2.zip) is also available for manual updates. Both are self-contained and currently unsigned, so Windows SmartScreen may warn on first launch.
